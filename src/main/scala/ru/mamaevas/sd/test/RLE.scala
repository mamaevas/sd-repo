...
object RLE {
  ...
  implicit class RleStringExt(s: String) {
    ...
    def rle: String = s.toSeq.rle

    def rlePar(grouped: Int = 1000, parallelism: Int = 10)
  ...
  implicit class RleLazyListExt(stream: Seq[Char]) {
    ...
    def rle: String = toRlePairs.map(RLE.toString).mkString
  }
  ...
  implicit class RleSourceExt(source: Source[Char, NotUsed]) {
    def toRlePairsAsync(grouped: Int = 1000, parallelism: Int = 10)
      ...
      source
        .grouped(grouped)
        .mapAsync(parallelism) { chSeq => Future(chSeq.toRlePairs) }
  ...
  private def toString(r: (Char, Int)) = s"${r._1}${if (r._2 == 1) "" else r._2}"
